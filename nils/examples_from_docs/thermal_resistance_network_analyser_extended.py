...
##############
Descr = "R_1,7"
Resistances.append(conv(None, 1, 6, h*2, A_1))
...
#Make Plot
############################
x=[L*1.2,0,L_1,L_2,L_2,L_2,-L*0.2]
y=[0,0,0,w/4,-w/4,0,0]
...
ax.plot([x[1],x[-1]],[y[1],y[-1]],'r--')
ax.set_yticks([])
ax.set_xticks([])

#%% Check node and system enthalpy conservation equations

def check_node_enthalpy_conservation(problem: ThermalNetworkProblem, analyzer: ThermalNetworkAnalyzer): # Q_dot: list, Resistances: list):

    Q_tot_in = sum(problem.Q_dot)
    Q_tot_out = 0
    
    print('len(analyzer.T) =', len(analyzer.T))

    for node_idx in range(problem.N_nodes):
        Q_gen = problem.Q_dot[node_idx]
    
        Q_flows = []
        for r in problem.res:
            print('r.Node2, r.Node1 =', r.Node2, r.Node1)
            
            if node_idx == r.Node1:
                # print(r.Descr)  # check whether correct resistances have been assigned
                Q_flow = (analyzer.T[r.Node2] - analyzer.T[r.Node1]) / r.resistance_value
                Q_flows.append(Q_flow)
            elif node_idx == r.Node2:
                # print(r.Descr)  # check whether correct resistances have been assigned
                Q_flow = (analyzer.T[r.Node1] - analyzer.T[r.Node2]) / r.resistance_value
                Q_flows.append(Q_flow)
            else:
                continue
...
check_node_enthalpy_conservation(prob, ana)
